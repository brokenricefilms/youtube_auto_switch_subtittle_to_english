...
(function () {
  ...
  const selectSubtitle = async (player, response) => {
    ...
    if (lang) {
      ...
      lang = captions
        .filter((o) => o.languageCode.startsWith("en"))
        .sort(
          (a, b) => (b.kind == "asr" ? 0 : 1) - (a.kind == "asr" ? 0 : 1)
        )[0]?.languageCode;
      if (lang) {
        console.log(lang + " -> en");
  ...
  unsafeWindow.addEventListener(
    ...
    () => {
      const firstResp =
        unsafeWindow?.ytplayer?.config?.args?.raw_player_response;
      if (firstResp) {
        load(firstResp);
      }
    },
